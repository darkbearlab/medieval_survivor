...
| 波次 | 週期 | 敵人組合 |
|------|------|---------|
| 1, 4, 7… | 白天 1 | 強盜（Bandit）|
| 2, 5, 8… | 白天 2 | 強盜 60% + 弓箭手 40%（Wave 9+ 加入 12% 法師）|
| 3, 6, 9… | 夜晚 | 強盜 45% + 弓箭手 30% + 重甲 25%（Wave 9+ 加入 20% 法師）|

每波敵人數：`3 + wave × 3`；夜晚波次乘 ×1.8。
...
- [x] 多種敵人類型（弓箭手 Archer、重甲兵 Heavy）
- [x] A* 尋路（PathFinder.js，64×64 網格）
- [x] 不可破壞地形（岩石群，8 組 L 形配置）
- [x] 玩家碰撞城牆/箭塔/地形/村莊中心

### Phase 4 — 功能擴充 ✅ 完成（2026-03-25）
- [x] 弓箭手投射物與建築碰撞並造成傷害
- [x] 鐵匠鋪（+5 玩家防禦）、訓練場（+10 玩家攻擊）
- [x] 食堂（在附近每秒回復 3HP）
- [x] 法師敵人（發射爆炸魔法彈，範圍傷害 65px，從第三晚 Wave 9 出現）
- [x] 波次縮放加強（每波 +3 敵人，原為 +2）

### Phase 5 — 自動化建築 ⏳ 待開始
> 規劃文件：[NEXT_UPDATE_PLAN.md](NEXT_UPDATE_PLAN.md)
- [ ] 採集所（自動採集範圍內重生的資源節點）
- [ ] 維修工（自動修復耐久最低的建築，8HP/s）

### Phase 6 — 兵營系統 ⏳ 待開始（Phase 5 之後）
> 規劃文件：[NEXT_UPDATE_PLAN.md](NEXT_UPDATE_PLAN.md)（Appendix 章節）
- [ ] 兵營（定時累積士兵，支付金幣部署）
- [ ] 士兵 AI（近戰/遠程隨機，Leash 機制，跟隨玩家作戰）
...
**實作內容：**
- **弓箭手投射物 vs 建築**：弓箭手發射的投射物現在與城牆/箭塔/鐵匠鋪/訓練場/食堂發生碰撞並造成傷害
- **鐵匠鋪**：費用 木材×15 石材×10，HP 120，放置後永久增加玩家防禦 +5（被摧毀後撤銷）
- **訓練場**：費用 木材×12 石材×8，HP 100，放置後永久增加玩家攻擊 +10（被摧毀後撤銷）
- **食堂**：費用 木材×10 石材×5，HP 90，玩家在 100px 範圍內每秒回復 3HP
- **法師敵人**：HP 45，發射慢速紫色魔法彈，命中後在 65px 範圍內爆炸（傷害玩家+建築），從 Wave 9（第三個夜晚）起開始出現
- **波次縮放加強**：每波敵人增量從 +2 提升至 +3

**架構決策記錄：**
- 法師投射物用獨立群組 `mageProjectiles`，碰撞建築觸發 `_triggerExplosion(x, y, dmg)` 呼叫 `_showExplosion()` 並對範圍內所有目標扣血
- 法師彈 vs 玩家仍用手動距離判定（同弓箭手彈，避免 Phaser 3.60 group vs sprite overlap 的 body 生命週期 bug）
- 食堂療癒在 `GameScene.update()` 逐幀累積 `healRate * delta / 1000`

*最後更新：2026-03-25（Phase 4）*
